...
exports.view_products = function(req, res) {
	...
	if(req.query.format=="xml") res.status(400).json({"success":false,"message":"XML format is not supported !"});
		...
		if(isNaN(req.query.start)) res.status(400).json({"success":false,"message":"Start variable must be an integer !"});
		else if(isNaN(req.query.count)) res.status(400).json({"success":false,"message":"Count variable must be an integer !"});
