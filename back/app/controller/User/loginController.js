...
exports.login_as_user = function(req, res) {
	...
	else if(! (req.body.psswd && req.body.username) ) {
	...
	else if(typeof req.body.username!='string' ||typeof req.body.psswd!='string' ) {
		res.status(400).json({"success":false,"message":"One or more fields are not valid !"});
	}
	else {
		if(validateEmail(req.body.username)) flag=1; 
		login(req.body.username , req.body.psswd ,flag, function (err,usr) {
